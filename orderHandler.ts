  ...
  discountFunctions = {
    menuPack: ({ packs }: { packs: { items: number[]; price: number }[] }) => {
      ...
      packs.forEach((pack) => {
        ...
        // If all items in the menu pack are present in current order
        if (menu.length === pack.items.length) {
          ...
          const min = Math.min(...this.items.map((i) => i.quantity));
    ...
    xFor1: ({ items, X }: { items: number[]; X: number }) => {
      ...
      items.forEach((item) => {
        ...
        if (index !== -1) {
          ...
          const qtty = this.items[index].quantity;
          ...
          total += integer * this.items[index].price;
          this.items[index].quantity = reminder;
        }
      });
      return total;
    },
